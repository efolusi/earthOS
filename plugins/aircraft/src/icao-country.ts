/**
 * ICAO 24-bit address (Mode S / ADS-B `hex`) -> country of registry.
 *
 * ICAO allocates contiguous 24-bit address blocks to states (Annex 10 Vol III).
 * The transponder hex is stamped at registration, so it is a reliable country
 * signal that is present on every aircraft worldwide, independent of the data
 * source (airplanes.live carries no country field; OpenSky carries its own, so
 * this table is only consulted when a source omits it).
 *
 * Ranges are inclusive [start, end] over the 24-bit address space and are kept
 * sorted by `start` so a lookup is a binary search. Values are the canonical
 * ICAO allocation; unallocated gaps resolve to '' (unknown), never a guess.
 */

export interface IcaoRange {
  /** inclusive lower bound, 24-bit */
  start: number;
  /** inclusive upper bound, 24-bit */
  end: number;
  country: string;
}

// Sorted by `start`. Covers the full ICAO nationality allocation.
// prettier-ignore
const RANGES: IcaoRange[] = [
  ...
  { start: 0x038000, end: 0x038fff, country: 'Ivory Coast' },
  ...
  { start: 0x078000, end: 0x0783ff, country: 'Somalia' },
  ...
  { start: 0x08c000, end: 0x08c3ff, country: 'DR Congo' },
  ...
  { start: 0xc8c000, end: 0xc8c3ff, country: 'Kiribati' },
  { start: 0xc8d000, end: 0xc8d3ff, country: 'Tonga' },
  { start: 0xc8e000, end: 0xc8e3ff, country: 'Tuvalu' },
...
/**
 * Country of registry for an ICAO 24-bit address in hex (e.g. `"ac308f"`).
 * Returns '' when the address is unallocated or unparseable, so callers can
 * fall back rather than display a wrong flag.
 */
export function icaoHexToCountry(hex: string | undefined | null): string {
  if (!hex) return '';
